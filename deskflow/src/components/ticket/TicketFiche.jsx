...
import { Button, Spinner, Badge, Modal } from '../templates';
...
export default function TicketFiche({ ticketId, open, onClose, onSaved }) {
  ...
  return (
    <Modal
      title={ticketData ? `${safeString(ticketData.name)} #${safeString(ticketData.id)}` : `Détails du Ticket #${ticketId}`}
      open={open}
      onClose={onClose}
      className="!max-w-6xl w-[90vw] !max-h-[85vh] flex flex-col"
    >
      {loading ? (
        <div className="p-10 flex justify-center items-center h-[50vh]"><Spinner /></div>
      ) : (
        <div className="flex flex-1 overflow-hidden min-h-0 h-[70vh] text-black font-sans">
          {/* Left Sidebar */}
          <div className="w-56 border-r border-gray-200 bg-gray-50 flex flex-col p-4 justify-between select-none">
            ...
            <Button variant="outline" className="border-black text-black hover:bg-black hover:text-white" onClick={onClose}>
              Fermer
            </Button>
          </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {timelineEvents.map((event, index) => (
                    <div key={index} className={`flex ${event.type === 'followup' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] border p-4 rounded-lg shadow-sm
