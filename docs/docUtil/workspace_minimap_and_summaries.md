...
│   ├── scenario_probable_budget_alertes.md  <-- Alerte budget & coûts associés d'un équipement (SQLite + React)
│   ├── scenarios_supplementaires_sqlite.md  <-- Système de favoris, assignation auto, notes privées
│   ├── ticket_status_history_sqlite.md      <-- Double écriture historique statuts GLPI + SQLite
│   ├── filtrage_general_date.md             <-- Filtre par plage de dates et reset sous React
...
### 📁 Thème A : Kanban, Filtrage & Interaction Utilisateur

| Fichier Guide | Résumé de la Fonctionnalité Décrite | Fichiers du Code Concernés |
| :--- | :--- | :--- |
| [ticket_filtering_guide.md](file:///d:/shania/itu/L3/glpi/docs/ticket_filtering_guide.md) | Guide pour implémenter des filtres de recherche multi-critères (par type, priorité, urgence, impact) sur la liste des tickets. | `TicketList.jsx` ou `TicketKanban.jsx` |
| [filtrage_general_date.md](file:///d:/shania/itu/L3/glpi/docs/filtrage_general_date.md) | **[NOUVEAU]** Concept de filtrage déclaratif en React pour trier des données par plage de dates (Min/Max) avec gestion du fuseau horaire et bouton de réinitialisation. | Filtres génériques React, Dashboard |
...
| [add_followup_scenario.md](file:///d:/shania/itu/L3/glpi/docs/add_followup_scenario.md) | Ajout de suivis et tâches techniques à un ticket existant (formulaire d'ajout rapide). | `TicketDetailView.jsx`, `TicketFiche.jsx` |
| [add_ticket_from_asset_guide.md](file:///d:/shania/itu/L3/glpi/docs/add_ticket_from_asset_guide.md) | Permet de créer un ticket d'incident pré-rempli directement depuis l'écran de visualisation d'un équipement informatique (`Asset`). | `AssetList.jsx`, `TicketCreate.jsx` |
...
| [business_logic_utility_functions.md](file:///d:/shania/itu/L3/glpi/docs/business_logic_utility_functions.md) | Regroupement des utilitaires numériques et de validation (`sanitizeNumber`, `calculateTimeCost`, `sumTicketCosts`, `buildDateTime`). | Services financiers & dates |
...
| [filtered_asset_stats_guide.md](file:///d:/shania/itu/L3/glpi/docs/filtered_asset_stats_guide.md) | Agrégation statistique pour afficher l'impact des incidents (nombre de tickets) par équipement informatique, filtrable par statut de ticket. | `Dashboard.jsx`, `FilteredAssetsTable.jsx` |
| [statistics_aggregation_guide.md](file:///d:/shania/itu/L3/glpi/docs/statistics_aggregation_guide.md) | Guide méthodologique pour la construction d'agrégations personnalisées (Regroupements par utilisateur, par type ou par période). | Dashboards de statistiques |
| [dashboardAvecTicket.md](file:///d:/shania/itu/L3/glpi/docs/dashboardAvecTicket.md) | Guide pour concevoir des graphes et indicateurs de performance financiers détaillés sur le Dashboard principal. | `Dashboard.jsx` |
...
| [scenarios_supplementaires_sqlite.md](file:///d:/shania/itu/L3/glpi/docs/scenarios_supplementaires_sqlite.md) | 3 scénarios avancés : Système de Favoris (Bookmarks), Moteur d'Assignation Automatique par Catégorie et Historique des Notes Privées Techniques. | SQLite Modèles, Repositories, Contrôleurs |
| [scenario_probable_budget_alertes.md](file:///d:/shania/itu/L3/glpi/docs/scenario_probable_budget_alertes.md) | **[NOUVEAU]** Système complet de contrôle budgétaire de maintenance par équipement avec enregistrement automatique d'alertes en BDD lors de dépassements de seuils. | `AssetBudget.java`, `BudgetController.java`, UI React |
| [ticket_status_history_sqlite.md](file:///d:/shania/itu/L3/glpi/docs/ticket_status_history_sqlite.md) | Mise en place d'une double écriture pour conserver localement l'historique complet des transitions de statut d'un ticket (du statut Nouveau à Clos). | `TicketStatusHistory.java`, `useTicket.js` |
...
| [TODO-login-mes-tickets.md](file:///d:/shania/itu/L3/glpi/docs/TODO-login-mes-tickets.md) | Scénario complet liant l'authentification SQLite au filtrage dynamique pour n'afficher que les tickets de l'utilisateur connecté. | `useTicket.js`, `Login.jsx` |
| [historique_statuts_complet.md](file:///d:/shania/itu/L3/glpi/docs/historique_statuts_complet.md) | Spécification sur le traçage des temps passés sur chaque statut de ticket et rapport d'audit dans le backoffice. | Rapports d'activité, SQLite |
| [reset_configuration.md](file:///d:/shania/itu/L3/glpi/docs/reset_configuration.md) | API backend et bouton frontend pour réinitialiser les configurations de couleurs et langues à l'état usine initial. | `DatabaseResetController.java`, `BackOffice.jsx` |
| [requetes_sqlite_services.md](file:///d:/shania/itu/L3/glpi/docs/requetes_sqlite_services.md) | **[NOUVEAU]** Dictionnaire complet de requêtes SQL (JPQL/Natives/JdbcTemplate) utiles à intégrer dans les services et repositories du backend Spring Boot. | Services, Repositories, Contrôleurs SQLite |
| [service_logique_metier_sqlite.md](file:///d:/shania/itu/L3/glpi/docs/service_logique_metier_sqlite.md) | **[NOUVEAU]** Implémentation Java complète d'une classe Spring Boot `@Service` centralisant les calculs financiers (TCO, ROI, efficacité) côté serveur. | `FinancialMetricsService.java`, DTOs, `FinancialMetricsController.java` |
| [bibliotheque_requetes_sql_entites.md](file:///d:/shania/itu/L3/glpi/docs/bibliotheque_requetes_sql_entites.md) | **[NOUVEAU]** Liste exhaustive des requêtes SQL SQLite (`SELECT *`, `GROUP BY`, `SUM`, `COUNT`, etc.) classées par entité pour l'évaluation. | Toutes les tables SQLite |
...
| [additional_opportunities_guide.md](file:///d:/shania/itu/L3/glpi/docs/additional_opportunities_guide.md) | Opportunités d'améliorations et vérifications d'intégrité lors de la synchronisation d'équipements en doublons dans GLPI. | `importService.js` |
| [support_langue_malgache_import.md](file:///d:/shania/itu/L3/glpi/docs/support_langue_malgache_import.md) | **[NOUVEAU]** Guide complet détaillant comment mapper les statuts, types, priorités et équipements rédigés en langue malgache dans l'import CSV. | `importService.js`, `DatabaseSeeder.java` |
| [centralisation_logique_couts.md](file:///d:/shania/itu/L3/glpi/docs/centralisation_logique_couts.md) | **[NOUVEAU]** Guide de centralisation et de découplage de la logique métier des coûts (Supercout, réouverture, annulation) entre le Kanban et l'import CSV. | `coutWorkflowService.js`, `importSqlite.js`, Modales Kanban |
| [scenarios_examen_import_logique_centralisee.md](file:///d:/shania/itu/L3/glpi/docs/scenarios_examen_import_logique_centralisee.md) | **[NOUVEAU]** Fiche de révision contenant 4 scénarios d'examen clés combinant l'import CSV, les coûts, l'historisation des statuts, les alertes budgétaires et le rollback. | Services d'import, `coutWorkflowService.js` |
...
| [scenarios_react_evaluation.md](file:///d:/shania/itu/L3/glpi/docs/scenarios_react_evaluation.md) | Ensemble complet de scénarios types pour le frontend React : gestion d'onglets dynamiques, formulaires de création, et modales d'actions. | Interface utilisateur Deskflow |
